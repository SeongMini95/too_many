...
public class ReviewService {
    ...
    public Long writeReview(Long userId, WriteReviewRequestDto requestDto) {
        ...
        PlaceInfo placeInfo = kakaoPlaceClient.getPlaceInfo(requestDto.getPlaceId());
        PlaceList placeList = kakaoMapsClient.getPlaceList(SearchStoreListRequestDto.builder()
                .query(placeInfo.getPlaceName())
                .x(requestDto.getX())
                .y(requestDto.getY())
                .page(1)
                .build(), true);
        ...
        // 카테고리 저장 및 가져오기
        int categoryDepth = placeList.getDepth();
        String lastCategoryName = placeList.getLastCategoryName();
        ...
        if (category == null) { // 카테고리가 없으면 저장
            String[] categoryNames = placeList.getCategoryNames();
        ...
        Store store = storeRepository.findByKakaoPlaceId(placeInfo.getPlaceId()).orElse(null);
        Store saveStore = Store.builder()
                .kakaoPlaceId(placeInfo.getPlaceId())
                .category(category)
                .regionCode(regionCode)
                .storeName(placeInfo.getPlaceName())
                .addressName(placeInfo.getAddress())
                .roadAddressName(placeInfo.getRoadAddress())
                .x(placeInfo.getX())
                .y(placeInfo.getY())
                .build();
        if (store != null) {
